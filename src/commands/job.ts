...
async function listJobs(args: string[], ctx?: RepoContext): Promise<string> {
  ...
  const ghArgs = [
    'job', 'list',
    '--json', 'id,name,status,created_at,stage',
    '--per-page', perPage,
  ];
  ...
  const suggestions = getSuggestions({ domain: 'job', action: 'list', isEmpty, repo: ctx });
  return renderOutput([
    countLine,
...
async function viewJob(args: string[], ctx?: RepoContext): Promise<string> {
  ...
  if (!id) throw new AxiError('Job ID is required: glab-axi job view <id>', 'VALIDATION_ERROR');

  const job = await glabJson<Record<string, unknown>>(
    ['job', 'view', id, '--json', 'id,name,status,stage,ref,created_at,started_at,web_url'],
    ctx,
  );
...
async function jobLog(args: string[], ctx?: RepoContext): Promise<string> {
  const positionals = args.filter((a) => !a.startsWith('--'));
  const id = positionals[1];
  if (!id) throw new AxiError('Job ID is required: glab-axi job log <id>', 'VALIDATION_ERROR');
...
async function retryJob(args: string[], ctx?: RepoContext): Promise<string> {
  const positionals = args.filter((a) => !a.startsWith('--'));
  const id = positionals[1];
  if (!id) throw new AxiError('Job ID is required: glab-axi job retry <id>', 'VALIDATION_ERROR');

  await glabExec(['job', 'retry', id], ctx);
  return renderOutput([
    encode({ retry: 'ok', job: id }),
    renderHelp([
      `Run \`glab-axi job log ${id}\` to monitor progress`,
    ]),
  ]);
}

export async function jobCommand(args: string[], ctx?: RepoContext): Promise<string> {
  const sub = args[0];
