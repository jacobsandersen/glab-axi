...
export async function apiCommand(args: string[], ctx?: RepoContext): Promise<string> {
  ...
  if (positionals.length >= 2 && HTTP_METHODS.has(positionals[0].toUpperCase())) {
    ...
    throw new AxiError('API path is required: glab-axi api [<method>] <path>', 'VALIDATION_ERROR');
  }
...
const NOISY_KEYS = new Set([
  ...
  'comments_url', 'timeline_url', 'node_id', 'url', 'repository_url',
  'html_url', 'reactions', 'user_view_type', 'site_admin',
...
function stripNoisyFields(obj: unknown, depth = 0): unknown {
  ...
  if (obj !== null && typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(record)) {
      ...
      if (key === 'user' && value && typeof value === 'object' && 'username' in (value as Record<string, unknown>)) {
