...
async function viewProject(args: string[], ctx?: RepoContext): Promise<string> {
  ...
  ghArgs.push('--json', 'path_with_namespace,name,description,visibility,star_count,forks_count,owner,default_branch,last_activity_at');
...
async function listProjects(args: string[], ctx?: RepoContext): Promise<string> {
  const perPage = getFlag(args, '--per-page') ?? '20';
  const ghArgs = [
    'project', 'list',
    '--json', 'path_with_namespace,name,visibility,star_count,last_activity_at',
    '--per-page', perPage,
  ];
  ...
  const countLine = formatCountLine({ count: projects.length, limit: limitNum });
  const suggestions = getSuggestions({ domain: 'project', action: 'list', isEmpty, repo: ctx });
  return renderOutput([
    countLine,
...
async function createProject(args: string[], ctx?: RepoContext): Promise<string> {
  const positionals = args.filter((a) => !a.startsWith('--'));
  const name = positionals[1];
  if (!name) throw new AxiError('Project name is required: glab-axi project create <name>', 'VALIDATION_ERROR');
  ...
  return renderOutput([
    ...
    renderHelp(getSuggestions({ domain: 'project', action: 'create', repo: ctx })),
  ]);
}
...
async function forkProject(args: string[], ctx?: RepoContext): Promise<string> {
  ...
  return renderOutput([
    ...
    renderHelp(getSuggestions({ domain: 'project', action: 'fork', repo: ctx })),
  ]);
}

export async function projectCommand(args: string[], ctx?: RepoContext): Promise<string> {
  const sub = args[0];
