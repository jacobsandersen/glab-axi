...
1. Run \`npx -y glab-axi\` with no arguments for a dashboard of the current project - open issues, merge requests, and suggested next commands.
2. Drill in command-first: \`issue list\`, \`issue view <n>\`, \`mr view <n>\`, \`pipeline list\`, \`job list --pipeline <id>\`, and so on.
3. Target another project by placing \`-R owner/name\`, \`-R=owner/name\`, \`--repo owner/name\`, or \`--repo=owner/name\` AFTER the command, e.g. \`npx -y glab-axi issue list --repo=owner/name\`.
